const scriptURL = 'https://script.google.com/macros/s/AKfycby6qdymuYyngHImwPzoGFr-BwNjrIWm-zmSRTsgTtpLQF-vM7N7t4zvFhLvQXmdlIr4/exec';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (form) {
        form.addEventListener('submit', e => {
            ...
            const formattedDate = coffeeChatValue ? coffeeChatValue.replace('T', ' ') : '미선택';
            ...
            const data = {
                ...
                '창업현황': statusVal === '기타' ? `기타(${formData.get('statusEtc') || ''})` : statusVal,
                '창업분야': formData.get('field'),
                '매출여부': revenueVal === '기타' ? `기타(${formData.get('revenueEtc') || ''})` : (revenueVal || '미선택'),
    ...
    document.querySelectorAll('.faq-question').forEach(q => {
        q.addEventListener('click', () => {
            ...
            if (toggle) toggle.innerText = item.classList.contains('active') ? '-' : '+';
        });
    });
    ...
    // Toast Notification
    const mockUsers = ["웰니스 분야 이*서님", "AI·딥테크 분야 박*준님", "에듀테크 분야 최*아님", "핀테크 서비스 김*현님"];
    function showNotification() {
        const toast = document.getElementById('notification-toast');
        const userText = document.getElementById('toast-user');
        if (toast && userText) {
            userText.innerText = mockUsers[Math.floor(Math.random() * mockUsers.length)];
                ...
                if (isDragging) {
                    ...
                    if (originalOnClick) {
                        ...
                        if (typeof originalOnClick === 'function') {
                            ...
                            const hrefMatch = card.getAttribute('onclick')?.match(/location\.href='([^']+)'/);
    ...
    // Expert Slider Buttons Click Logic
    ...
    if (slider && nextBtn && prevBtn) {
        const getScrollAmount = () => {
            const card = slider.querySelector('.expert-card');
            return card ? card.offsetWidth + parseInt(getComputedStyle(slider).gap) : 300;
        };
